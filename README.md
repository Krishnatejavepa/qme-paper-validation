...
- `consistency_check.py` prints `58/58 PASS` and exits 0.
...
Three scripts are kept for provenance and do not run from a clean checkout:
`s3b_litexp.py` needs a Materials Project API key and the trained model weights,
`d1_offset_analysis.py` needs the calculation engine and its job output, and
`bib_verify.py` needs network access. The numbers in the paper come from the
committed outputs listed above, so none of these three is needed to reproduce
them.

## Calibration audit (Section VI)

The paper registers an offset audit of our own PBE+U bench against four benchmark
Li couples and reports its design and decision rule, but no result: those
calculations were still running at submission, so no offset number appears in
v1. The verdict, and a matching update to this repository, will follow in a
later version. The design and the gate are in
`prereg/D1_PREREGISTRATION_2026-06-10.md`.
